...
extract_snp_groups = function(pheno_gwas, endo_gwas, map, thresh, filter_hapmap, hapmap){
  ...
  snps_assoc = table(df_combined$SNP)
  snps_assoc[snps_assoc == 2] = "both"
  snps_assoc[snps_assoc == 1 & names(snps_assoc) %in% df_pheno$SNP] = "pheno_only"
  snps_assoc[snps_assoc == 1 & names(snps_assoc) %in% df_endo$SNP] = "endo_only"
